...
import { TInitiative, TWorkBreakdown, InitiativeStatus, TProjectVitalsAdvanced } from '../../../types';
...
const SmartGanttChart: React.FC<{ data: TProjectVitalsAdvanced, onUpdateTask?: (taskId: string, start: number, duration: number) => void }> = ({ data, onUpdateTask }) => {
    ...
    const dayWidth = 40;
    const maxDays = Array.isArray(data?.tasks) && data.tasks.length > 0 
        ? Math.max(...data.tasks.map(t => (t.start || 0) + (t.duration || 1))) + 2 
        : 10;
    ...
    const sortedTasks = useMemo(() => 
        (Array.isArray(data?.tasks) ? data.tasks : []).slice().sort((a, b) => (a.start || 0) - (b.start || 0))
    , [data?.tasks]);
                            ...
                            <div className="relative flex-grow">
                                ...
                                <div 
                                    className={`absolute top-[8px] h-[24px] rounded-full shadow-sm text-[10px] flex items-center px-2 text-white overflow-hidden whitespace-nowrap transition-colors hover:opacity-90 cursor-move ${
                                        isDone ? 'bg-accent-emerald' : task.isCritical ? 'bg-accent-red ring-2 ring-accent-red/30' : 'bg-accent-purple'
                                    }`}
                                    style={{ 
                                ...
                                >
                                    {progress > 0 && <div className="absolute left-0 top-0 bottom-0 bg-black/20 pointer-events-none" style={{ width: `${progress}%` }} />}
...
const ResourceHeatmap: React.FC<{ resources: TProjectVitalsAdvanced['resources'], tasks: TProjectVitalsAdvanced['tasks'] }> = ({ resources, tasks }) => {
    ...
    if (safeResources.length === 0) return <div className="text-gray-500 text-sm">{t('projectHub:noResourceData')}</div>;

    const maxDays = safeTasks.length > 0 ? Math.max(...safeTasks.map(t => (t.start || 0) + (t.duration || 1))) + 2 : 10;
    const workload: Record<string, number[]> = {};
    safeResources.forEach(res => workload[res.assignee] = Array(maxDays).fill(0));
    ...
    safeTasks.forEach(task => {
        if (task.assignee && workload[task.assignee]) {
            ...
            for (let i = start; i < start + duration; i++) {
                if (i < maxDays) workload[task.assignee][i] += 1;
            }
        }
    });
                    ...
                    return (
                        <div key={res.assignee} className="bg-surface-light dark:bg-surface-dark p-4 rounded-lg shadow-sm border border-gray-200 dark:border-border-dark">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`text-xs font-bold px-2 py-0.5 rounded-full text-white ${color}`}>{res.utilization}%</span>
                            </div>
            ...
            <div className="bg-surface-light dark:bg-surface-darker border border-gray-200 dark:border-border-dark rounded-lg overflow-x-auto custom-scrollbar">
                <div className="min-w-max">
                    <div className="flex border-b border-gray-200 dark:border-border-dark">
                        ...
                        {Array.from({ length: maxDays }).map((_, i) => (
                            <div key={i} className="w-10 p-2 text-center text-xs font-medium text-gray-500 border-r border-gray-100 dark:border-border-dark">D{i + 1}</div>
                        ))}
                    </div>
                    {safeResources.map(res => (
                        <div key={res.assignee} className="flex border-b border-gray-100 dark:border-border-dark last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            <div className="w-48 p-3 text-sm font-medium text-gray-900 dark:text-gray-100 bg-surface-light dark:bg-surface-darker sticky left-0 z-10 border-r border-gray-200 dark:border-border-dark flex items-center">{res.assignee}</div>
                            {workload[res.assignee].map((count, i) => (
                                <div key={i} className="w-10 p-1 border-r border-gray-100 dark:border-border-dark">
                                    <div className={`w-full h-full min-h-[28px] rounded flex items-center justify-center text-xs font-bold transition-colors ${getHeatmapColor(count)}`}>{count > 0 ? count : ''}</div>
                                </div>
                            ))}
                        </div>
                    ))}
...
export const ProjectHub: React.FC<ProjectHubProps> = ({ initiatives, onSaveWbs, initialSelectedInitiativeId }) => {
    ...
    const handleAdvancedAnalysis = async (init: TInitiative, wbsData: TWorkBreakdown) => {
        setIsLoading(true);
        try {
            const result = await analyzeCriticalPath(JSON.stringify(wbsData), init.sector);
    ...
    const handleSelectInitiative = (id: string) => {
        ...
        if (initiative) {
            ...
            if (initiative.wbs) handleAdvancedAnalysis(initiative, initiative.wbs);
        }
    };
    ...
    const handleGenerateWbs = async () => {
        ...
        setIsLoading(true);
        try {
            const result = await generateWbs(selectedInitiative, selectedInitiative.artifacts?.backlog || MOCK_BACKLOG);
            setWbs(result);
    ...
    const handleAutoPrioritize = async () => {
        ...
        try {
            ...
            setAdvancedVitals({
                ...advancedVitals,
                tasks: (Array.isArray(advancedVitals.tasks) ? advancedVitals.tasks : []).map(t => {
    ...
    const handleGetOptimizationAdvice = async () => {
        ...
        try {
            const context = `Initiative: ${selectedInitiative.title}\nCritical Path: ${advancedVitals.criticalPathDuration} days\nRisk Analysis: ${advancedVitals.riskAnalysis}`;
            setOptimizationAdvice(await generateOptimizationAdvice(context, selectedInitiative.sector));
    ...
    const groupedInitiatives = useMemo(() => {
        const approved = (initiatives || []).filter(i => 
            [InitiativeStatus.IN_DEVELOPMENT, InitiativeStatus.LIVE, InitiativeStatus.AWAITING_APPROVAL].includes(i.status)
        );
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 mt-4 sm:mt-0">
                    ...
                    {advancedVitals && selectedInitiative && (
                        <Button onClick={handleAutoPrioritize} disabled={isLoading} variant="outline" className="border-accent-purple/20 text-accent-purple hover:bg-accent-purple/10">
                            <Zap className="h-4 w-4 mr-2"/> {t('projectHub:autoPrioritize')}
                        </Button>
                    )}
                    ...
                    {wbs && selectedInitiative && (
                        <Button onClick={() => onSaveWbs(selectedInitiative.id, wbs)} variant="primary" className="bg-accent-emerald hover:bg-accent-emerald/80">
                           <Download className="h-5 w-5 mr-2"/> {t('projectHub:save')}
                        </Button>
                    )}
                </div>
            </div>
                    ...
                    <div className="space-y-6 animate-fade-in-down">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="md:col-span-2 bg-accent-purple/10 p-6 rounded-lg border border-accent-purple/20 relative overflow-hidden">
                                <h3 className="font-bold text-accent-purple flex items-center gap-2 mb-2"><Sparkles className="h-5 w-5" /> {t('projectHub:aiRiskAnalysis')}</h3>
                        ...
                        <div className="bg-accent-amber/10 border border-accent-amber/20 rounded-xl p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-bold text-accent-amber flex items-center gap-2"><Lightbulb className="h-6 w-6" /> {t('projectHub:strategicOptimizationAdvice')}</h3>
                                {!optimizationAdvice && (
                                    <Button onClick={handleGetOptimizationAdvice} disabled={isOptimizing} variant="outline" className="border-accent-amber/30 text-accent-amber hover:bg-accent-amber/10">
                            ...
                            {optimizationAdvice ? (
                                <div className="prose prose-sm dark:prose-invert max-w-none text-accent-amber">
                                    ...
                                    <div className="mt-4 flex justify-end">
                                        <Button onClick={() => setOptimizationAdvice(null)} variant="ghost" className="text-accent-amber"> {t('projectHub:dismiss')} </Button>
                                    </div>
                                </div>
                            ) : <p className="text-sm text-accent-amber/70 italic">{t('projectHub:optimizationAdviceDesc')}</p>}
                        </div>

                        <div className="toggle-bar-container mb-6">
                            <button onClick={() => setActiveTab('Schedule')} className={`toggle-item ${activeTab === 'Schedule' ? 'toggle-item-active' : 'toggle-item-inactive'}`}>{t('projectHub:intelligentSchedule')}</button>
                            <button onClick={() => setActiveTab('Resources')} className={`toggle-item ${activeTab === 'Resources' ? 'toggle-item-active' : 'toggle-item-inactive'}`}>{t('projectHub:resourceHeatmap')}</button>
                        </div>

                        {activeTab === 'Schedule' ? <SmartGanttChart data={advancedVitals} onUpdateTask={(taskId, start, duration) => {
                            const updatedTasks = (Array.isArray(advancedVitals.tasks) ? advancedVitals.tasks : []).map(t => t.id === taskId ? { ...t, start, duration } : t);
                            setAdvancedVitals({ ...advancedVitals, tasks: updatedTasks });
                        }} /> : <ResourceHeatmap resources={advancedVitals.resources} tasks={advancedVitals.tasks} />}
                    </div>
                )
            )}
        </div>
