...
export const OracleView: React.FC<OracleViewProps> = ({ initiatives, onSelectInitiative }) => {
    const { askOracle, isAIProcessing, aiError } = useAI();
    ...
    const handleSearch = async (q: string) => {
        if (!q.trim()) return;
        try {
            const result = await askOracle(q, initiatives);
    ...
    return (
        <div className="flex flex-col h-full bg-surface-light dark:bg-surface-dark text-text-main-light dark:text-text-main-dark rounded-2xl overflow-hidden relative font-sans animate-in fade-in duration-700">
            {/* Ambient Background */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-accent-teal/5 via-transparent to-transparent pointer-events-none"></div>
            ...
            <div className="p-8 border-b border-border-light dark:border-border-dark z-10 flex items-center justify-between bg-surface-light/50 dark:bg-surface-dark/50 backdrop-blur-md">
                <div>
                    <h1 className="text-3xl font-black flex items-center gap-4 tracking-tight">
                        <div className="p-2 bg-accent-teal/10 rounded-2xl">
                            <Sparkles className="h-8 w-8 text-accent-teal" />
                        </div>
                        <span className="bg-clip-text text-transparent bg-gradient-to-r from-accent-teal to-accent-teal/60">The Oracle</span>
                    </h1>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-12 z-10 custom-scrollbar" ref={scrollRef}>
                {history.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-center animate-in zoom-in-95 duration-700">
                        <div className="p-10 bg-accent-teal/5 rounded-full mb-8">
                            <Search className="h-24 w-24 text-accent-teal/30" />
                        </div>
                        ...
                        <p className="text-sm text-text-muted-light dark:text-text-muted-dark max-w-md mt-4 leading-relaxed font-medium">I have read every requirement, risk, and decision. Try asking about <span className="text-accent-teal">"Security Risks in Fintech"</span> or <span className="text-accent-teal">"Shared Tech Stack"</span>.</p>
                    </div>
                )}
                ...
                {history.map((entry, i) => (
                    <div key={i} className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex justify-end">
                            <div className="bg-accent-teal text-primary rounded-3xl rounded-tr-none px-8 py-4 max-w-2xl text-lg font-medium shadow-xl shadow-accent-teal/20">
                                {entry.query}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-6">
                            <div className="flex-shrink-0 mt-1">
                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-accent-teal to-accent-teal/80 flex items-center justify-center shadow-xl shadow-accent-teal/30 transform rotate-3">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                        {entry.response.citations.map((cit, j) => (
                                            <div 
                                                ...
                                                className="bg-surface-light dark:bg-surface-darker/50 border border-border-light dark:border-border-dark hover:border-accent-teal/50 p-5 rounded-2xl cursor-pointer transition-all group hover:shadow-xl hover:-translate-y-1"
                                            >
                                                <div className="flex items-center gap-3 mb-3">
                                                    <div className="p-2 bg-accent-teal/10 rounded-lg group-hover:bg-accent-teal group-hover:text-primary transition-colors">
                                                ...
                                                <div className="flex items-center gap-2 mb-3">
                                                    <BookOpen className="h-3 w-3 text-accent-teal" />
                                                    <p className="text-[10px] font-bold text-accent-teal uppercase tracking-wider">{cit.artifactType}</p>
                                                </div>
                                                <div className="relative">
                                                    <Quote className="h-4 w-4 text-accent-teal/20 absolute -top-2 -left-2" />
                                    ...
                                    <div className="flex flex-wrap gap-3 mt-6">
                                        {entry.response.suggestedFollowUps.map((suggestion, k) => (
                                            <button 
                                                ...
                                                className="px-5 py-2.5 rounded-full border border-border-light dark:border-border-dark hover:border-accent-teal hover:bg-accent-teal/5 bg-surface-light dark:bg-surface-darker/50 text-xs font-bold text-text-main-light dark:text-text-main-dark transition-all flex items-center gap-2 group"
                                            >
                                                {suggestion}
                ...
                {isAIProcessing && (
                    <div className="flex gap-6 animate-in fade-in duration-500">
                        <div className="flex-shrink-0 mt-1">
                            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-accent-teal to-accent-teal/80 flex items-center justify-center animate-pulse shadow-xl shadow-accent-teal/30">
                ...
                <div className="relative max-w-4xl mx-auto group">
                    ...
                    <div className="relative">
                        <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                            <MessageSquare className="h-5 w-5 text-accent-teal opacity-50 group-focus-within:opacity-100 transition-opacity" />
                        </div>
                        <input
                            type="text"
                            value={query}
                            ...
                            className="w-full bg-surface-light dark:bg-surface-darker border border-border-light dark:border-border-dark text-text-main-light dark:text-text-main-dark rounded-3xl py-5 pl-16 pr-20 focus:outline-none focus:ring-4 focus:ring-accent-teal/10 focus:border-accent-teal transition-all shadow-xl text-lg font-medium placeholder:text-text-muted-light/30 dark:placeholder:text-text-muted-dark/30"
                            autoFocus
                        />
                        <button 
                            onClick={() => handleSearch(query)}
                            disabled={!query.trim() || isAIProcessing}
                            className="absolute right-3 top-1/2 -translate-y-1/2 p-4 bg-accent-teal text-primary rounded-2xl hover:bg-accent-teal/90 disabled:opacity-50 disabled:hover:bg-accent-teal transition-all shadow-lg shadow-accent-teal/20 group/btn active:scale-95"
                        >
                            {isAIProcessing ? <Spinner /> : <Send className="w-5 h-5 group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />}
                        </button>
                    </div>
                </div>
            </div>
