...
export const InitiativeForm: React.FC<InitiativeFormProps> = ({ initiative, projectId, onSubmit, onCancel }) => {
  ...
  const [readinessThreshold, setReadinessThreshold] = useState(initiative?.thresholds?.readiness || 0.6);
  const [financialThreshold, setFinancialThreshold] = useState(initiative?.thresholds?.financial || 0.9);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-8 bg-surface-light dark:bg-surface-dark rounded-2xl shadow-2xl border border-border-light dark:border-border-dark w-full max-w-lg animate-in fade-in zoom-in duration-300">
      ...
      <div className="space-y-4">
        ...
        <div className="pt-4 space-y-4 border-t border-border-light dark:border-border-dark">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    <div className="flex justify-between">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-text-muted-light dark:text-text-muted-dark ml-1">Readiness Risk</label>
                        <span className="text-[10px] font-bold text-accent-purple">{(readinessThreshold * 100).toFixed(0)}%</span>
                    </div>
                    <input 
                        type="range" 
                        min="0" 
                        ...
                        onChange={(e) => setReadinessThreshold(parseFloat(e.target.value))}
                        className="w-full accent-accent-purple cursor-pointer h-1.5 bg-surface-darker/10 rounded-lg appearance-none"
                    />
                    <p className="text-[9px] text-text-muted-light italic">Alert when score falls below this.</p>
                </div>
                
                <div className="space-y-2">
                    <div className="flex justify-between">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-text-muted-light dark:text-text-muted-dark ml-1">Budget Burn</label>
                        <span className="text-[10px] font-bold text-accent-red">{(financialThreshold * 100).toFixed(0)}%</span>
                    </div>
                    <input 
                        type="range" 
                        min="0" 
                        ...
                        className="w-full accent-accent-red cursor-pointer h-1.5 bg-surface-darker/10 rounded-lg appearance-none"
                    />
                    <p className="text-[9px] text-text-muted-light italic">Alert when spend exceeds this.</p>
                </div>
            </div>
        </div>
      </div>
