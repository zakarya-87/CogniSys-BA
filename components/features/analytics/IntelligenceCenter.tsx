...
import { useAI } from '../../../context/CatalystContext';
...
const KpiCard: React.FC<{ kpi: TKpi; onClick: () => void; isSelected: boolean }> = ({ kpi, onClick, isSelected }) => {
    ...
    const trend = (trendConfig as any)[kpi.trend];
...
export const IntelligenceCenter: React.FC<{ initiatives: TInitiative[]; onCreateInitiative: (title: string, description: string, sector?: any) => void; }> = ({ initiatives, onCreateInitiative }) => {
    ...
    const { getRecommendations, getForecast, getDailyBriefing, getKpiInsights, isAIProcessing, aiError } = useAI();
    ...
    useEffect(() => {
        const fetchBriefing = async () => {
            try {
                const result = await getDailyBriefing(initiatives, i18n.language);
                setBriefing(result);
            } catch (e) {
                console.error(e);
            }
    ...
    }, [initiatives, i18n.language, getDailyBriefing]);

    const handleGenerate = async () => {
        setRecommendations(null);
        try {
            const result = await getRecommendations(initiatives, mockKpis, Sector.GENERAL, i18n.language);
    ...
    const handleKpiClick = async (kpi: TKpi) => {
        if (selectedKpi?.name === kpi.name) return;
        setSelectedKpi(kpi);
        setIsForecasting(true);
        setForecast(null);
        setKpiInsights(null);
        try {
            const forecastPromise = getForecast(kpi.name, kpi.value, 'Technology & SaaS', i18n.language); 
            const insightsPromise = getKpiInsights(kpi, i18n.language);
    ...
    const [activeSubTab, setActiveSubTab] = useState<'Insights' | 'Graph'>('Insights');
    ...
    return (
        <div className="p-8 space-y-12 max-w-[1600px] mx-auto animate-in fade-in duration-700">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-6 mt-4">
                        <button 
                            ...
                            className={`text-lg font-bold tracking-tight pb-2 border-b-4 transition-all ${activeSubTab === 'Insights' ? 'border-accent-purple text-text-main-light dark:text-text-main-dark' : 'border-transparent text-text-muted-light dark:text-text-muted-dark hover:text-text-main-light'}`}
                        >
                            Deep Insights
                        </button>
                        <button 
                            onClick={() => setActiveSubTab('Graph')}
                            className={`text-lg font-bold tracking-tight pb-2 border-b-4 transition-all ${activeSubTab === 'Graph' ? 'border-accent-purple text-text-main-light dark:text-text-main-dark' : 'border-transparent text-text-muted-light dark:text-text-muted-dark hover:text-text-main-light'}`}
                        >
                            Portfolio Navigator
                        </button>
                    </div>
            ...
            {activeSubTab === 'Graph' ? (
                <div className="bg-surface-light dark:bg-surface-dark rounded-[2.5rem] p-10 shadow-sm border border-border-light dark:border-border-dark min-h-[600px] flex flex-col relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                        <Sparkles className="w-64 h-64 text-accent-purple" />
                    </div>
                    <div className="flex justify-between items-center mb-10 relative z-10">
                        <div>
                            <h2 className="text-3xl font-black tracking-tighter">Strategic Dependency Graph</h2>
                            <p className="text-sm font-medium text-text-muted-light dark:text-text-muted-dark mt-1">AI-mapped relationships across portfolio initiatives</p>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex items-center gap-2 px-4 py-2 bg-surface-darker/5 dark:bg-surface-darker/20 rounded-xl border border-border-light dark:border-border-dark">
                                <div className="w-3 h-3 rounded-full bg-accent-purple" />
                                <span className="text-[10px] font-black uppercase tracking-widest text-text-muted-light">Initiative</span>
                            </div>
                            <div className="flex items-center gap-2 px-4 py-2 bg-surface-darker/5 dark:bg-surface-darker/20 rounded-xl border border-border-light dark:border-border-dark">
                                <div className="w-3 h-3 rounded-full bg-accent-emerald" />
                                <span className="text-[10px] font-black uppercase tracking-widest text-text-muted-light">Active</span>
                            </div>
                        </div>
                    </div>
                    
                    <div className="flex-1 bg-surface-darker/5 dark:bg-surface-darker/40 rounded-3xl border border-border-light dark:border-border-dark relative group overflow-hidden">
                         <div className="absolute inset-0 flex items-center justify-center">
                             {/* In a real scenario, we would use d3 here. For now, let's create a beautiful static representation */}
                             <div className="w-full h-full flex items-center justify-center p-20">
                                <div className="relative w-full h-full">
                                   {initiatives.slice(0, 8).map((init, i) => {
                                       const angle = (i / Math.min(initiatives.length, 8)) * 2 * Math.PI;
                                       const x = 50 + 35 * Math.cos(angle);
                                       const y = 50 + 35 * Math.sin(angle);
                                       return (
                                           <React.Fragment key={init.id}>
                                               <div 
                                                    className="absolute w-2 h-2 rounded-full bg-accent-purple/20 animate-pulse"
                                                    style={{ left: '50%', top: '50%', width: '400px', height: '1px', transform: `rotate(${angle}rad)`, transformOrigin: '0 0' }}
                                               />
                                               <div 
                                                   className="absolute p-4 bg-white dark:bg-surface-dark rounded-2xl border border-accent-purple/30 shadow-2xl transition-all duration-500 hover:scale-110 hover:border-accent-purple hover:z-20 cursor-pointer group/node"
                                                   style={{ left: `${x}%`, top: `${y}%`, transform: 'translate(-50%, -50%)' }}
                                               >
                                                   <div className="flex items-center gap-3">
                                                       <div className={`w-3 h-3 rounded-full ${init.status === 'Live' ? 'bg-accent-emerald' : 'bg-accent-purple'} shadow-lg group-hover/node:scale-125 transition-transform`} />
                                                       <span className="text-xs font-black tracking-tight whitespace-nowrap">{init.title}</span>
                                                   </div>
                                               </div>
                                           </React.Fragment>
                                       );
                                   })}
                                   <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-accent-purple/10 rounded-full flex items-center justify-center border-4 border-accent-purple/20 animate-pulse">
                                       <Brain className="w-12 h-12 text-accent-purple" />
                                   </div>
                                </div>
                             </div>
                         </div>
                         <div className="absolute bottom-10 left-10 p-6 bg-white/80 dark:bg-surface-dark/80 backdrop-blur-xl rounded-2xl border border-border-light dark:border-border-dark max-w-xs shadow-2xl">
                             <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-2 text-accent-purple">Topology Insight</h4>
                             <p className="text-xs font-bold leading-relaxed">{t('intelligenceCenter.topologyExplanation', { defaultValue: 'Your portfolio shows high centralisation around core tech initiatives. AI suggests diversifying risk by decoupling legacy dependencies.' })}</p>
                         </div>
                    </div>
                </div>
            ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-10 rounded-[2.5rem] shadow-sm border border-border-light dark:border-border-dark">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-8 mb-12">
                        ...
                        <Button onClick={handleGenerate} disabled={isAIProcessing} className="px-10 py-4 rounded-2xl shadow-2xl hover:shadow-accent-purple/40 transition-all active:scale-95 font-black uppercase tracking-widest text-xs">
                            {isAIProcessing ? <Spinner /> : <><RefreshCw className="h-5 w-5 mr-3 animate-spin-slow" /> {t('intelligenceCenter.generate')}</>}
                        </Button>
                    </div>
                    ...
                    {aiError && (
                        <div className="p-4 bg-accent-red/10 border border-accent-red/20 rounded-2xl mb-8 animate-in shake duration-500">
                            <p className="text-accent-red text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
                                <ShieldAlert className="h-4 w-4" /> {aiError}
                            </p>
                        </div>
                    )}
